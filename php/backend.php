<?php
    ...
    #changed based on file structure in my localhost
	...
	{   
	    ...
	    #tables names and database name were edited based on the mock_test_tbl.sql file
	...
	Class ManageApp {
		...
		function MyConnection (&$connection,$host,$user,$db)
		{
	        ...
	        #root password changed based on database connection
        	...
        	$extra_slots_entry= mysqli_prepared_query($connection_mock_chat,$query,"ii",$params);
		        if($extra_slots_entry)
		        {
		            foreach ($extra_slots_entry as $val)
		            {
		                ...
		                #column names are changed based on sql file and database table
		                $Name=$val['Name'];
		                $Email=$val['Email'];
		                $phoneNum=$val['Phone'];
		                $Gender=$val['Gender'];
